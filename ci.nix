{ config, pkgs, lib, ... }: with pkgs; with lib; let
  ...
  artifacts = "${artifactRoot}/lib/arcdps_example_addon*.dll";
  release = "${artifactRoot}/lib/arcdps_example_addon.dll";
in
{
  config = {
    name = "arcdps_example_addon";
    ci.gh-actions = {
      enable = true;
      export = true;
    };
    ...
    artifactPackage = runCommand "example-artifacts" { } (''
      mkdir -p $out/lib
      cp ${config.artifactPackages.main}/lib/arcdps_example_addon.dll $out/lib/
    '' + concatStringsSep "\n" (mapAttrsToList (key: addonPath: ''
        cp ${addonPath}/lib/arcdps_example_addon.dll $out/lib/arcdps_example_addon-${key}.dll
